...
func main() {
	...
	// Do attempts the func at most once. The command timeout defaults to 1000ms
	// (configurable via CommandConfig.Timeout). Errors, timeouts and circuit rejection
	// invoke FallbackFunc if provided. Timeout neither cancels the running func nor
	// limits the fallback's duration: Do can remain blocked while the fallback runs.
	// Bound any blocking work inside the fallback separately.
