...
## Quickstart
From `sys/xxhash3`, regenerate both SIMD assembly files with Go 1.20 or newer:
```sh
cd internal/avo && ./build.sh
```
The POSIX shell script writes `sys/xxhash3/avx2_amd64.s` and
`sys/xxhash3/sse2_amd64.s`. It uses the separate tool module's pinned
`github.com/mmcloughlin/avo v0.2.0`; it does not need GNU sed or change the
library module's dependencies.

Verify generation in a temporary directory without overwriting the checked-in
assembly (run from `sys/xxhash3/internal/avo`):
```sh
go test -run TestGeneratedAssembly -count=1
```
This compares every non-comment assembly line with the checked-in files.
Generated command comments and leading blank lines differ, so whole-file byte
equality is not expected. Run the hash consumer tests separately from the
repository root:
```sh
go test ./sys/xxhash3
```
SIMD execution requires amd64 and the corresponding CPU feature; generation on
another architecture does not establish SIMD runtime correctness.
