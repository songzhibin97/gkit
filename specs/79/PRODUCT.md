...
8. Group publication initializes every member as pending before publishing any member. If pending initialization fails, no member is published. Cancellation before any publisher starts rolls back the group and every initialized member so the same group can be retried. Once any publisher starts, the method joins all started publishers, preserves every confirmed successful publication, moves each failed or never-admitted member to a terminal failure, returns a stable result slice, and deterministically reports publication, cancellation, and convergence errors. A publication failure or cancellation must not leave a member permanently pending solely because it was never queued.
