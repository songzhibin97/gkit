...
func TestPretty(t *testing.T) {
	...
	opts := *DefaultOptions
	opts.SortKeys = true
	pretty = BestOptions(Ugly(Pretty([]byte(example2))), &opts)
	assertEqual(t, j(pretty), j(pretty))
	assertEqual(t, j(example2), j(pretty))
}
...
func TestBig(t *testing.T) {
  ...
  },
  {
	...
	opts := *DefaultOptions
	opts.SortKeys = true
	jsonb := BestOptions(Ugly([]byte(js)), &opts)
	assertEqual(t, j(jsonb), j(js))
}
...
func TestPrettyStableSort(t *testing.T) {
	...
	opts := *DefaultOptions
	opts.SortKeys = true
	json = string(Ugly(BestOptions([]byte(json), &opts)))
...
func TestSpec(t *testing.T) {
  ...
  {    	     
	...
	out := string(Spec([]byte(js)))
	if out != expect {
		t.Fatalf("expected '%s', got '%s'", expect, out)
	}
	out = string(SpecInPlace([]byte(js)))
	if out != expect {
		t.Fatalf("expected '%s', got '%s'", expect, out)
	}
}
