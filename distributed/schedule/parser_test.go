...
func TestOptionalSecondSchedule(t *testing.T) {
	parser := NewParse(WithInterval(SecondOptional | Minute | Hour | Dom | Month | Dow | Descriptor))
...
func TestStandardSpecSchedule(t *testing.T) {
	...
	for _, c := range entries {
		actual, err := NewParseWithStandard(c.expr)
		if len(c.err) != 0 && (err == nil || !strings.Contains(err.Error(), c.err)) {
			t.Errorf("%s => expected %v, got %v", c.expr, c.err, err)
		}
		if len(c.err) == 0 && err != nil {
...
func TestNoDescriptorParser(t *testing.T) {
	parser := NewParse(WithInterval(Minute | Hour))
